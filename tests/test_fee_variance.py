...
def test_fee_adjusted_match_uses_percentage_tolerance():
	gateway = pd.DataFrame(
		[{"order_id": "ORD-1", "amount": 1000.0, "created_at": "2026-01-01"}]
	)
	bank = pd.DataFrame(
		[{"utr": "UTR-1", "settled_amount": 1004.0, "settlement_date": "2026-01-02", "batch_reference": "ORD-1"}]
	)

	assert len(fee_adjusted_match(gateway, bank)) == 1
